...
lbl-1000:
        ...
        {
            while (true) {
                if ((line = reader.readLine()) == null) ** GOTO lbl58
...
lbl58:
                    ...
                    if (currentKey == null) return count;
                    if (currentLines.isEmpty() != false) return count;
                    count += DialogueLoader.registerDialogue(byKey, currentKey, currentWeight, currentTags, v1Parts, v2Parts, currentLines, cultureId, currentBuildings, currentNotBuildings, currentVillagers, currentNotVillagers, currentRelations, currentNotRelations, lang, isOverlay, priorSubmodKeys, recordNewKeysInto, sourceLabel);
                    return count;
                }
                var25_29 = params.split(",");
                var26_30 = var25_29.length;
                var27_31 = 0;
                while (true) {
                    if (var27_31 >= var26_30) ** break;
                    param = var25_29[var27_31];
        ...
        if (byKey.containsKey(key)) {
            if (isOverlay) {
                if (priorSubmodKeys != null && priorSubmodKeys.contains(key)) {
                    if (WARNED_REPLACE_IDS.add("dialogue:" + lang + ":" + String.valueOf((Object)cultureId) + ":" + key)) {
    ...
    public static List<Dialogue> getDialogues(ResourceLocation culture, String lang) {
        ...
        Map<String, Dialogue> byCulture = byLang.get((Object)culture);
        if (byCulture == null) {
            return List.of();
        }
