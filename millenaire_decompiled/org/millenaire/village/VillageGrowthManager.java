...
public final class VillageGrowthManager {
    ...
    public static boolean rushOneProject(ServerLevel level, Village village, @Nullable VillageTerrainMap terrainMap, @Nullable Set<ResourceLocation> rushExcluded, @Nullable TerrainReachability reachability, @Nullable RushDispersion dispersion) {
        ...
        if (rushExcluded != null && !rushExcluded.isEmpty()) {
            candidates.removeIf(c -> rushExcluded.contains((Object)c.planSet().id()));
        }
        if (candidates.isEmpty()) {
            return false;
        }
    ...
    private static boolean hasResources(ServerLevel level, Village village, BuildingPlanSet.LevelDef levelDef, AffordabilityMode mode) {
        ...
        Integer anywoodNeeded = required.get((Object)AnywoodHelper.ANYWOOD_LOG);
    ...
    static boolean isOnPlacementCooldown(Village village, ResourceLocation planSetId, long currentTick) {
        Long until = village.getPlacementCooldowns().get((Object)planSetId);
    ...
    static List<PlacementStep> deduplicateTerrainSteps(List<PlacementStep> terrainSteps, List<PlacementStep> buildingSteps, int groundLevel) {
        ...
        for (PlacementStep step : terrainSteps) {
            if (!buildingPositions.contains((Object)step.relativePos())) {
                result.add(step);
                continue;
            }
    ...
    static final class AffordabilityMode
    extends Enum<AffordabilityMode> {
        public static final /* enum */ AffordabilityMode RAW_COUNT = new AffordabilityMode();
        public static final /* enum */ AffordabilityMode RESERVATION_AWARE = new AffordabilityMode();
        private static final /* synthetic */ AffordabilityMode[] $VALUES;

        public static AffordabilityMode[] values() {
            return (AffordabilityMode[])$VALUES.clone();
        }

        public static AffordabilityMode valueOf(String name) {
            return Enum.valueOf(AffordabilityMode.class, name);
        }

        private static /* synthetic */ AffordabilityMode[] $values() {
            return new AffordabilityMode[]{RAW_COUNT, RESERVATION_AWARE};
        }

        static {
            $VALUES = AffordabilityMode.$values();
        }
    }
}
