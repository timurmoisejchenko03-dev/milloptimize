...
public final class GoodAvailabilityHelper {
    ...
    public static int getConstructionReservedQuantity(Item item, ServerLevel level, Village village, @Nullable Village.PendingProject excludeProject) {
        ...
        for (BuildingInstance building : village.getBuildings()) {
            ...
            if (resCost2.containsKey((Object)itemId)) {
                int needed = resCost2.get((Object)itemId);
                int builderHas = 0;
        ...
        if (pending != null && !pendingHandled && !pending.equals(excludeProject) && (planSet = ModCultures.getBuildingPlanSet(pending.planSetId())) != null && (pendingDef = planSet.getLevel(variant = pending.variant(), pendingLevel = pending.level())) != null && (resCost = pendingDef.requiredResources()).containsKey((Object)itemId)) {
            reserved += resCost.get((Object)itemId).intValue();
        }
        return reserved;
    }
    ...
    public static int getAnywoodReservedQuantity(ServerLevel level, Village village, @Nullable Village.PendingProject excludeProject) {
        ...
        for (BuildingInstance building : village.getBuildings()) {
            ...
            Integer anywoodNeeded2 = resCost.get((Object)AnywoodHelper.ANYWOOD_LOG);
        ...
        if (pending != null && !pendingHandled && !pending.equals(excludeProject) && (planSet = ModCultures.getBuildingPlanSet(pending.planSetId())) != null && (pendingDef = planSet.getLevel(variant = pending.variant(), pendingLevel = pending.level())) != null && (anywoodNeeded = pendingDef.requiredResources().get((Object)AnywoodHelper.ANYWOOD_LOG)) != null) {
    ...
    private static int getResidentNeeds(BuildingInstance building, Item item, Village village) {
        ...
        for (VillagerRecord record : village.getVillagerRecords().values()) {
            ...
            if (homeId == null || !homeId.equals(building.getId()) || (vtype = ModCultures.getVillagerType(record.getVillagerTypeId())) == null || (qty = vtype.resolvedRequiredGoods().get((Object)item)) == null) continue;
            needs += qty.intValue();
        }
        return needs;
    }
